...
def login():
    ...
    # 记录收到的表单数据
    current_app.logger.info(f"登录请求 - 用户名: '{username}', 密码长度: {len(password)}")

    # 使用统一验证函数
    error = validate_user_input(username, password)
    if error:
        current_app.logger.info(f"登录验证失败 - 错误信息: {error}")
