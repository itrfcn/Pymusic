...
def update_single_user(user_id):
    ...
    try:
        with Mysql() as mysql:
            ...
            data = request.get_json()
            if not data:
                return jsonify({'message': '没有提供要修改的字段'}), 400
            ...
            for field, value in data.items():
                if field in allowed_fields:
                    ...
                    if field == 'is_admin':
                        if value not in [0, 1]:
                            return jsonify({'message': 'is_admin字段必须是0或1'}), 400
                        update_fields.append(f"{field} = %s")
                        update_values.append(value)
                    # 特殊处理status字段，确保它是整数0或1
                    elif field == 'status':
                        if value not in [0, 1]:
                            return jsonify({'message': 'status字段必须是0或1'}), 400
                        update_fields.append(f"{field} = %s")
                        update_values.append(value)
                    # 处理username字段，确保不为空
                    elif field == 'username':
            ...
            if not update_fields:
                return jsonify({'message': '没有提供有效的字段进行修改'}), 400
...
def delete_playlist(playlist_id):
    ...
    try:
        with Mysql() as mysql:
            ...
            if not playlist:
                return jsonify({'message': '歌单不存在'}), 404
            ...
            return jsonify({'message': '删除歌单成功'}), 200
...
def remove_admin(user_id):
    ...
    try:
        with Mysql() as mysql:
            ...
            if admin_count <= 1:
                return jsonify({'message': '不能解除最后一个管理员的权限'}), 400
            ...
            return jsonify({'message': '解除管理员权限成功'}), 200
...
def add_user():
    ...
    try:
        ...
        data = request.get_json()
        if not data:
            return jsonify({'message': '没有提供用户信息'}), 400
